...
bool oled_task_user(void) {
    oled_write_ln_P(PSTR("BomboPad v0.0"), false);
    switch (current_layer) {
        case _BASE:
            oled_write_ln_P(PSTR("Layer: Default"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR(" Esc   FullSc  PgUp "), false);
            oled_write_ln_P(PSTR("CA + T Slide  PgDown"), false);
            oled_write_ln_P(PSTR(" Ctrl   Draw   Clr  "), false);
            break;
        case _XPP:
            oled_write_ln_P(PSTR("Layer: Xournal++"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR(" SReg.  SRet.   Del "), false);
            oled_write_ln_P(PSTR(" Pen   HiLgtr Eraser"), false);
            oled_write_ln_P(PSTR(" Text   Undo   Redo "), false);
            break;
        case _DPP:
            oled_write_ln_P(PSTR("Layer: Canon DPP"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR("Check   Main   Edit "), false);
            oled_write_ln_P(PSTR("RevSht  Save   Undo  "), false);
            oled_write_ln_P(PSTR("RevSvd ------  Redo "), false);
           break;
        case _SCH:
            oled_write_ln_P(PSTR("Layer: Schematics"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR(" Line   Conn  Power "), false);
            oled_write_ln_P(PSTR(" Label Symbol  Wire "), false);
            oled_write_ln_P(PSTR(" Shift  Undo   Redo "), false);
            break;
        case _PCB:
            oled_write_ln_P(PSTR("Layer: PCB"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR("Tracks SelTrk DelTrk"), false);
            oled_write_ln_P(PSTR(" Area   Vias  ------"), false);
            oled_write_ln_P(PSTR(" Prop  RelPos AbsPos"), false);
            break;
        case _XXX:
            oled_write_ln_P(PSTR("Layer: Experimental"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR("------ ------ ------"), false);
            oled_write_ln_P(PSTR("------ ------ ------"), false);
            oled_write_ln_P(PSTR("------ ------ ------"), false);
            break;
        case _NUM:
            oled_write_ln_P(PSTR("Layer: Numeric"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            ...
            break;
        case _OBS:
            oled_write_ln_P(PSTR("Layer: OBS"), false);
            oled_write_ln_P(PSTR("------|------|------"), false);
            oled_write_ln_P(PSTR("Stream Record Camera"), false);
            oled_write_ln_P(PSTR(" Pres   Desk  DskPrs"), false);
            oled_write_ln_P(PSTR(" Capt  DskCpt PrsCpt"), false);
            break;
        default:
